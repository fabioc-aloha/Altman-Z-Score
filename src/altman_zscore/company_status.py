...
BANKRUPTCY_INDICATORS = [
    'bankruptcy', 'chapter 11', 'chapter 7', 'liquidation', 
    'restructuring', 'insolvency', 'bankrupt', 'receivership',
    'filing for protection', 'debt restructuring'
]
...
def check_company_status(ticker: str) -> CompanyStatus:
    ...
    if not status.is_active and not status.is_delisted and not status.is_bankrupt:
        try:
            ...
            sec_url = f"https://www.sec.gov/cgi-bin/browse-edgar?CIK={ticker}&Find=Search&owner=exclude&action=getcompany"
