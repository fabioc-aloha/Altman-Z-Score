"""
company_status_helpers.py
------------------------
Helpers for company_status.py: region detection, special status handling, and related utilities.

This module provides functions to detect company region, handle special status cases (bankruptcy,
delisting, inactivity), and check company status using Yahoo Finance and SEC data. It also defines
constants for known bankruptcies and bankruptcy indicators.
"""
...
def detect_company_region(info: dict) -> str:
    ...
    Args:
        info (dict): Info dictionary from Yahoo Finance or SEC.
    Returns:
        str: Region string (e.g., 'US', 'EM', 'EU', 'ASIA', etc.) or 'Unknown'.
    """
    if not info:
        return "Unknown"
...
def handle_special_status(status) -> bool:
    ...
    Creates appropriate marker files and error outputs for reporting and downstream logic.
...
def check_company_status(ticker: str, CompanyStatusClass=None) -> 'CompanyStatus':
    ...
    Uses multiple methods to detect special situations, including yfinance and SEC lookups.
