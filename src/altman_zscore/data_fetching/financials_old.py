...
def fetch_financials(ticker: str, end_date: str, zscore_model: str, start_date: str = None) -> Optional[Dict[str, Any]]:
    ...
    if "sales" not in fields_to_fetch:
        fields_to_fetch.append("sales")    # --- SEC EDGAR primary ---
    sec_quarters = []
    try:
        from altman_zscore.api.sec_client import SECClient
        from altman_zscore.utils.sec_facts_processor import extract_quarterly_financials_from_sec_facts
        ...
        # Process SEC facts into quarterly financials
        if sec_facts and sec_facts.get('facts'):
            logger.info(f"[{ticker}] Processing SEC facts into quarterly financials")
            sec_financials = extract_quarterly_financials_from_sec_facts(
                sec_facts, zscore_model, ticker, start_date, end_date
            )
            if sec_financials and sec_financials.get('quarters'):
                sec_quarters = sec_financials['quarters']
                logger.info(f"[{ticker}] Extracted {len(sec_quarters)} quarters from SEC facts")
                
                # Save SEC quarterly data
                try:
                    with open(os.path.join(output_dir, "financials_sec_quarterly.json"), "w", encoding="utf-8") as f:
                        json.dump(sec_quarters, f, indent=2, ensure_ascii=False, default=str)
                except Exception as e:
                    logger.warning(f"[{ticker}] Could not save SEC quarterly financials: {e}")
            else:
                logger.warning(f"[{ticker}] Could not extract quarterly financials from SEC facts")
        else:
            logger.warning(f"[{ticker}] No SEC company facts; will use yfinance fallback.")
    except Exception as sec_e:
        logger.info(f"[{ticker}] SEC EDGAR processing failed: {sec_e}. Will use yfinance fallback.")        
