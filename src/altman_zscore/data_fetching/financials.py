...
def fetch_financials(ticker: str, end_date: str, zscore_model: str):
    ...
    if ai_field_mapping:
        try:
            ...
            logger.warning(
                f"AI field mapping failed: {e}. All fields will be treated as missing."
            )
            ai_mapping = None
        ...
        if (not len(bs.columns) and not len(is_.columns)) or not common_periods:
            ...
            try:
                ...
                if not cik:
                    ...
                    raise ValueError(
                        f"No financial statement columns found for ticker '{ticker}'. The company may be delisted, never public, or data is missing from yfinance/SEC."
                    )
                ...
                logger.error(
                    f"[{ticker}] SEC fallback found some data, but full SEC XBRL parsing is not implemented in this pipeline. Please implement SEC XBRL parsing for full support."
                )
                raise ValueError(
                    f"SEC fallback found some data for ticker '{ticker}', but full SEC XBRL parsing is not implemented. Please add SEC XBRL support to proceed."
                )
        ...
        for period in common_periods:
            ...
            for key in critical_fields:
                ...
                if ai_mapping and key in ai_mapping and ai_mapping[key]:
                    ai_raw_field = (
                        ai_mapping[key]["FoundField"] if isinstance(ai_mapping[key], dict) else ai_mapping[key]
                    )
                found = False
            ...
            if missing:
                logger.warning(
                    f"[{ticker}] {period_str}: Missing fields: {', '.join(missing)}. Available BS keys: {sorted(available_bs_keys)}. Available IS keys: {sorted(available_is_keys)}"
                )
            quarters.append(q)
            # Track missing fields for each quarter for partial analysis and reporting
...
def find_xbrl_tag(soup, tag_names):
    """
    Search for the first tag in tag_names in the BeautifulSoup XBRL soup and return its value as float.
    Returns None if not found.
    """
    for tag in tag_names:
        el = soup.find(name=tag)
        if el and el.text:
            try:
