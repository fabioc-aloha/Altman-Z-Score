...
HTML_OUTPUT = os.path.join(BASE_DIR, "growth_picks.html")

def get_company_data():
    """
    Read all company summary and comprehensive reports to find suitable picks for growth investors.
    ...
    for ticker in company_dirs:
        ...
        try:
            ...
            # Check the recommendation for Growth profile in comprehensive report if available
            growth_rec = None
            if os.path.exists(report_path):
                try:
                    ...
                    # Look for Growth recommendation
                    growth_match = re.search(r'📈\s*(?:<strong>)?Growth(?:</strong>)?.*?<strong>(BUY|HOLD|SELL|STRONG BUY)</strong>', 
                                              report_content, re.DOTALL | re.IGNORECASE)
                    if growth_match:
                        growth_rec = growth_match.group(1).strip()
                except Exception as e:
                    logger.error(f"Error parsing comprehensive report for {ticker}: {str(e)}")
            
            # Growth investor criteria:
            # 1. Z-Score >= 3.0 (safe zone, financially sound)
            # 2. Growth recommendation is STRONG BUY or BUY (if available)
            # 3. General recommendation is not SELL
            
            if z_score >= 3.0 and recommendation.lower() != "sell":
                if growth_rec is not None and growth_rec.upper() in ["STRONG BUY", "BUY"]:
                    ...
                    company_data.append({
                        ...
                        "growth_rec": growth_rec
                    })
                    logger.info(f"Added {ticker} - {company_name} with Z-Score {z_score} for Growth profile")
...
def generate_html(company_data):
    """
    Generate HTML file with company cards for growth investors.
    """
    if not company_data:
...
<head>
    ...
    <title>Growth Investor Stock Picks - Altman Z-Score</title>
    <style>
        body {{
...
<body>
    <div class="header-container">
        <h1><span class="profile-icon">📈</span> Growth Investor Stock Picks</h1>
        <div class="date">{current_date}</div>
    </div>

    <div class="profile-description">
        <p><strong>Growth Investor Profile:</strong> Focused on capital appreciation with medium to high risk tolerance. 
        Growth investors prioritize companies with strong growth potential, increasing market share, and promising future prospects. 
        The stocks in this list combine solid financial health (Z-Score > 3.0) with growth characteristics and positive analyst recommendations.</p>
    </div>

    <div class="summary">
        <h2>Investment Summary</h2>
        <p>Based on the Altman Z-Score analysis and growth investor criteria, we have identified <strong>{company_count}</strong> companies that show strong growth potential while maintaining solid financial health. These companies combine high Z-Scores (good financial foundation) with favorable growth prospects.</p>
        ...
        <p>These recommendations are particularly suitable for investors focused on capital appreciation and growth potential.</p>
    </div>
    ...
    <div class="filters">
        <div class="filter-group">
            ...
            <select id="sort-by">
                ...
                <option value="rec-strongbuy">Strong Buy First</option>
            </select>
        </div>
        <div class="filter-group">
    ...
    for i, company in enumerate(company_data):
        ...
        top_label = ""
        if i == 0:
            top_label = '<span class="category-label">Top Growth Pick</span>'
        
        # Set recommendation class
        rec_class = company["growth_rec"].lower().replace(" ", "")
        ...
        <div class="company-card" data-ticker="{ticker}" data-z-score="{company["z_score"]}" data-rec="{rec_class}">
            ...
            <div class="label">Growth Investor:</div>
            <div class="recommendation {rec_class}">{company["growth_rec"]}</div>
    ...
    <div class="disclaimer">
        ...
        These recommendations are tailored for growth-focused investors and are derived from financial health metrics. 
        Growth stocks may be subject to higher volatility and risks. Always consult with a financial advisor before 
            ...
            function sortCompanies() {
                ...
                const sortedCompanies = [...companies].sort((a, b) => {
                    switch(sortValue) {
                        ...
                        case 'rec-strongbuy':
                            if (a.dataset.rec === 'strongbuy' && b.dataset.rec !== 'strongbuy') return -1;
                            if (a.dataset.rec !== 'strongbuy' && b.dataset.rec === 'strongbuy') return 1;
                            return parseFloat(b.dataset.zScore) - parseFloat(a.dataset.zScore);
                        default:
                            return 0;
...
def main():
    logger.info("Starting Growth Investor Picks Generator")
    
    # Get company data
    company_data = get_company_data()
    logger.info(f"Found {len(company_data)} companies matching Growth investor criteria")
