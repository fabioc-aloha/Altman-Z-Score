...
HTML_OUTPUT = os.path.join(BASE_DIR, "strong_buys.html")

def get_company_data():
    """
    Read all company summary files and extract Z-Score, risk category, and recommendations.
    Return only companies with 'Strong Buy' recommendations.
    """
    company_data = []
    ...
    for ticker in company_dirs:
        ...
        try:
            ...
            # Only include companies with "Strong Buy" recommendation (case insensitive)
            if recommendation.lower() == "strong buy":
                ...
                logger.info(f"Added {ticker} - {company_name} with Z-Score {z_score}")
...
def generate_html(company_data):
    """
    Generate HTML file with company cards.
    """
    if not company_data:
    ...
    top_companies = company_data[:3]
    top_company_text = f"{top_companies[0]['name']} ({top_companies[0]['ticker']})"
    second_company_text = f"{top_companies[1]['name']} ({top_companies[1]['ticker']})"
    third_company_text = f"{top_companies[2]['name']} ({top_companies[2]['ticker']})"
...
<head>
    ...
    <title>Altman Z-Score Strong Buy Recommendations</title>
    <style>
        body {{
...
<body>
    <div class="header-container">
        <h1>Altman Z-Score Strong Buy Recommendations</h1>
        <div class="date">{current_date}</div>
    </div>

    <div class="summary">
        <h2>Investment Summary</h2>
        <p>Based on the Altman Z-Score analysis, we have identified <strong>{company_count}</strong> companies with verified "STRONG BUY" recommendations. These companies demonstrate solid financial health with Z-Scores well above the safety threshold of 2.99, indicating low bankruptcy risk and strong fundamentals.</p>
        <p>The highest Z-Score belongs to <span class="highest-z-score">{top_company_text}</span> with an exceptional Z-Score of {top_companies[0]["z_score"]:.2f}, followed by <span class="highest-z-score">{second_company_text}</span> with {top_companies[1]["z_score"]:.2f} and <span class="highest-z-score">{third_company_text}</span> with {top_companies[2]["z_score"]:.2f}.</p>
        <p>These recommendations are particularly suitable for growth and value investors looking for financially sound companies with potential for capital appreciation.</p>
    </div>
    ...
    for i, company in enumerate(company_data):
        ...
        top_label = ""
        if i == 0:
            top_label = '<span class="category-label">Top Z-Score</span>'
        ...
        <div class="company-card" data-ticker="{ticker}" data-z-score="{company["z_score"]}">
            ...
            <div class="recommendation">{company["recommendation"]}</div>
    ...
    <div class="disclaimer">
        <p><strong>Disclaimer:</strong> This information is based on Altman Z-Score analysis as of the current date shown. The "Strong Buy" recommendations are derived from financial health metrics and do not guarantee future performance. These recommendations should be used as one of many factors in your investment decision process. Always consult with a financial advisor before making investment decisions. This analysis is not financial advice.</p>
    </div>
...
def main():
    logger.info("Starting Strong Buys HTML generator")
    
    # Get company data
    company_data = get_company_data()
    logger.info(f"Found {len(company_data)} companies with Strong Buy recommendations")
