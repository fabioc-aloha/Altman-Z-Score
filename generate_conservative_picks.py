...
HTML_OUTPUT = os.path.join(BASE_DIR, "conservative_picks.html")

def get_company_data():
    """
    Read all company summary and comprehensive reports to find suitable picks for conservative investors.
    Conservative investors prioritize:
    - Safety (high Z-Score in Safe category, > 2.99)
    ...
    for ticker in company_dirs:
        ...
        try:
            ...
            # 1. Z-Score >= 3.5 (well into the Safe zone for extra safety margin)
            # 2. Risk Category must be "Safe"
            # 3. Recommendation should be "Strong Buy" or "Buy" or "Hold"
            
            if z_score >= 3.5 and risk_category.lower() == "safe" and recommendation.lower() in ["strong buy", "buy", "hold"]:
                # Check the recommendation for Conservative profile in comprehensive report if available
                conservative_rec = "HOLD"  # Default
                if os.path.exists(report_path):
                    try:
                        with open(report_path, 'r', encoding='utf-8') as f:
                            report_content = f.read()
                        
                        # Look for Conservative recommendation
                        conservative_match = re.search(r'📊\s*<strong>Conservative</strong>.*?<strong>(BUY|HOLD|SELL)</strong>', 
                                                    report_content, re.DOTALL)
                        if conservative_match:
                            conservative_rec = conservative_match.group(1).strip()
                    except Exception as e:
                        logger.error(f"Error parsing comprehensive report for {ticker}: {str(e)}")
                
                # Only include if conservative recommendation is BUY or HOLD
                if conservative_rec.lower() in ["buy", "hold"]:
                    ...
                    company_data.append({
                        ...
                        "conservative_rec": conservative_rec
                    })
                    logger.info(f"Added {ticker} - {company_name} with Z-Score {z_score} for Conservative profile")
...
def generate_html(company_data):
    """
    Generate HTML file with company cards for conservative investors.
    """
    if not company_data:
...
<head>
    ...
    <title>Conservative Investor Stock Picks - Altman Z-Score</title>
    <style>
        body {{
...
<body>
    <div class="header-container">
        <h1><span class="profile-icon">📊</span> Conservative Investor Stock Picks</h1>
        <div class="date">{current_date}</div>
    </div>

    <div class="profile-description">
        <p><strong>Conservative Investor Profile:</strong> Focused on capital preservation with low risk tolerance. 
        These investors prioritize financial stability, low bankruptcy risk, and consistent performance over high returns. 
        The stocks in this list have high Z-Scores (3.5+) well into the "Safe" zone, providing an extra margin of safety.</p>
    </div>

    <div class="summary">
        <h2>Investment Summary</h2>
        <p>Based on the Altman Z-Score analysis and conservative investor criteria, we have identified <strong>{company_count}</strong> companies that meet strict safety requirements. These companies demonstrate exceptional financial health with Z-Scores well above the safety threshold of 2.99, indicating minimal bankruptcy risk and strong fundamentals.</p>
        <p>The highest Z-Score belongs to <span class="highest-z-score">{top_company_text}</span> with an exceptional Z-Score of {top_companies[0]["z_score"]:.2f} (if available), followed by <span class="highest-z-score">{second_company_text}</span> and <span class="highest-z-score">{third_company_text}</span>.</p>
        <p>These recommendations are particularly suitable for risk-averse investors focused on capital preservation and steady, reliable performance.</p>
    </div>
    ...
    for i, company in enumerate(company_data):
        ...
        top_label = ""
        if i == 0:
            top_label = '<span class="category-label">Top Safety Pick</span>'
            
        html_content += f'''
        <div class="company-card" data-ticker="{ticker}" data-z-score="{company["z_score"]}">
            {top_label}
            ...
            <div class="label">Conservative Investor:</div>
            <div class="recommendation">{company["conservative_rec"]}</div>
    ...
    <div class="disclaimer">
        ...
        These recommendations are tailored for conservative investors and are derived from financial health metrics. 
        They do not guarantee future performance. Always consult with a financial advisor before making investment decisions. 
        This analysis is not financial advice.</p>
    </div>
...
def main():
    logger.info("Starting Conservative Investor Picks Generator")
    
    # Get company data
    company_data = get_company_data()
    logger.info(f"Found {len(company_data)} companies matching Conservative investor criteria")
