...
> **DIAMOND v4.10.0** (July 12, 2025) | 427+ Companies | Enterprise-Scale Portfolio Management
...
- @unicodecorruption Rule - Fix Unicode File Corruption with Complete Replacement: When Unicode corruption creates complex file duplication and malformed content (like mixing "### 🚀 **Portfolio Efficiency Revolution**10.0 DIAMOND" text), attempting string replacements often fails due to exact text matching issues. Instead, create a clean version of the corrupted file and replace it entirely. This approach ensures complete corruption removal and prevents partial fixes that leave residual issues. Always verify corruption is fully eliminated using grep searches for the � character.
- @skipexisting Rule - Skip-Existing Feature Implementation: Both Python CLI (--skip-existing) and PowerShell (-SkipExisting) tools provide identical functionality for efficient large portfolio management. Use check_existing_analysis() in Python and Test-ExistingAnalysis in PowerShell to validate CSV, JSON, and report file existence with non-zero size checks. Essential for incremental portfolio updates and resuming interrupted analysis on large portfolios (427+ companies). Always provide progress reporting with original/skipped/remaining counts and early exit when all tickers have existing analysis.
- @fiscalyear Rule - Dynamic Fiscal Year Detection: Never use hardcoded fiscal year end mappings. Always implement dynamic fiscal year end detection using FMP API by examining recent financial statements (income statement, balance sheet). Cache the results to avoid repeated API calls. The fiscal year end determines the correct future dates for forecasting visualizations and ensures "year 1" forecasts refer to the current fiscal year (if not yet ended) or next fiscal year (if current has ended).
- @forecastyearlogic Rule - Forecast Year Semantic Consistency: Ensure forecast "year 1" always refers to the current fiscal year (if not yet ended) and "year 2" to the next fiscal year, regardless of what years analyst estimates are available for. Use company-specific fiscal year end dates to determine the correct target fiscal years and corresponding forecast dates for visualization. This provides intuitive user experience where forecasts are always relative to the company's business cycle.
- @forecastvisualization Rule - Forecast Timeline Visualization: When plotting forecast data on trend charts, use the company's fiscal year end dates to determine correct future plot points. Connect forecast lines from the last historical data point and ensure proper temporal spacing based on actual fiscal year intervals. Cache fiscal year end lookups to improve performance across multiple forecast scenarios.
- @analystagreement Rule - Analyst Estimate Year Mapping: Analyst estimates may be available for different fiscal years than what users expect for "year 1" and "year 2" forecasts. Implement robust mapping logic that translates user forecast years (1, 2) to available analyst estimate years based on company fiscal year calendars and current date context. Log this mapping for transparency and debugging.
...
- @componentprojection Rule - Robust Financial Component Projection: When projecting Z-Score components for forecasting, use backward-compatible field mapping that handles both real field names and legacy mock field names. Apply scenario-specific growth adjustments (optimistic +10%, base case 0%, pessimistic -10%) to revenue and EBITDA growth rates. Project each component proportionally: working capital ~50% of revenue growth, retained earnings ~30%, EBIT based on EBITDA estimates, market equity ~40%, asset turnover ~80%. Include retail-specific components like inventory turnover when applicable.
- @forecastarchitecture Rule - Scalable Forecast Architecture: Design forecasting systems with clear separation between data fetching (ConsensusFetcher), calculation (ZScoreForecaster), and visualization (TrendChart). Use async/await patterns for API calls and implement comprehensive error handling with graceful degradation. Cache fiscal year end lookups and provide detailed logging for debugging forecast year mapping. Always return rich ForecastResult objects with scenarios, metadata, and quality indicators.
- @consensusintegration Rule - Analyst Consensus Data Integration: When integrating analyst consensus data, validate data quality using coverage_quality scores and provide limited forecasts for low-quality data. Map analyst estimate fiscal years to user-expected forecast periods using company-specific fiscal year calendars. Log the mapping between available estimate years and target forecast years for transparency and debugging.
- @errorhandling Rule - Comprehensive Error Handling with User Guidance: Always implement comprehensive error handling that provides clear, actionable feedback to users. Include specific error messages that explain what went wrong, why it happened, and what steps users can take to resolve the issue. For financial data processing, include fallback mechanisms and suggest alternative data sources when primary sources fail. Never fail silently - always log errors and provide user-friendly explanations with suggested next steps.
- @datainjectionconsistency Rule - Ensure Z-Score Data Injection Consistency: Always ensure that the Z-Score values injected into the LLM match exactly with the values stored in the final JSON output. The "current" or "latest" Z-Score referenced in LLM analysis must be the same value stored in analysis_summary.z_score. Implement data validation to flag discrepancies between injected quarterly data and current summary values. Use consistent precision formatting across all Z-Score representations (e.g., always 3 decimal places for accuracy).
- @quarterylabeling Rule - Accurate Quarter Period Labeling in Data Injection: When injecting multi-quarter Z-Score data to the LLM, ensure period labels are accurate and clearly identify which quarter represents the "current" or "latest" calculation. Replace "QUnknown Unknown" labels with proper fiscal quarter/year identifiers (e.g., "Q1 2025", "Q2 2025") to prevent LLM confusion about which Z-Score represents the current financial state.
- @llmvalidation Rule - Validate LLM Referenced Values Against Source Data: After LLM analysis generation, implement validation checks to ensure all Z-Score values referenced in the LLM narrative match the source data. Flag any discrepancies between LLM-referenced values and actual calculated values for manual review. This prevents the LLM from inadvertently using rounded, outdated, or incorrect Z-Score values in its investment analysis.
- @logocachecleanup Rule - Delete Expired Cached Files Before Re-downloading: When cache TTL expires for any cached files (especially logos, images, or binary data), always delete the expired cached file before downloading a fresh copy. This prevents file system conflicts, ensures clean cache state, and avoids potential issues with partial downloads or file locks. Use try/catch blocks around file deletion and log both successful deletions and any deletion failures for debugging purposes.
